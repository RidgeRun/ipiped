/* ipiped.c generated by valac 0.11.5, the Vala compiler
...
gint ipipe_init_aew (Ipipe* self, const gchar* wb, const gchar* ae, const gchar* g, const gchar* meter, gint time, gint fps, gint segment_factor, gint width, gint height, GError** error);
...
Ipipe* ipipe_construct (GType object_type, const gchar* _sensor, const gchar* _video_processor, AbstcVideoProcessor* _video_processor_abstract, AbstcSensor* _sensor_abstract) {
	...
	gchar* _tmp2_;
	gchar* _tmp3_;
	AbstcVideoProcessor* _tmp4_;
	AbstcVideoProcessor* _tmp5_;
	AbstcSensor* _tmp6_;
	AbstcSensor* _tmp7_;
	...
	self->priv->sensor = _tmp1_;
	_tmp2_ = g_strdup (_video_processor);
	_tmp3_ = _tmp2_;
	_g_free0 (self->priv->video_processor);
	self->priv->video_processor = _tmp3_;
	_tmp4_ = _g_object_ref0 (_video_processor_abstract);
	_tmp5_ = _tmp4_;
	_g_object_unref0 (self->video_processor_abstract);
	self->video_processor_abstract = _tmp5_;
	_tmp6_ = _g_object_ref0 (_sensor_abstract);
	_tmp7_ = _tmp6_;
	_g_object_unref0 (self->sensor_abstract);
	self->sensor_abstract = _tmp7_;
	ipipe_debug = FALSE;
	return self;
}
...
/**
         * Ping
         * Show to the ipipe-client that an instance of the ipiped damon is 
         * alive
         */
gboolean ipipe_ping (Ipipe* self, GError** error) {
...
gint ipipe_init_aew (Ipipe* self, const gchar* wb, const gchar* ae, const gchar* g, const gchar* meter, gint time, gint fps, gint segment_factor, gint width, gint height, GError** error) {
	...
	if (g_strcmp0 (meter, "S") == 0) {
		meter_type = METER_SPOT;
	} else {
	...
	abstc_video_processor_get_video_processor_data (self->video_processor_abstract, &interf);
	...
	_tmp1_ = create_rraew (wb_algo, ae_algo, meter_type, width, height, fps, segment_factor, gain_type, &fd, &sensor, &interf);
	self->priv->rraew = _tmp1_;
	self->priv->aew_running = TRUE;
	self->priv->clean_close = TRUE;
	_tmp2_ = g_thread_supported ();
	if (!_tmp2_) {
		g_error ("ipiped.vala:216: Cannot run without thread support");
	}
	_tmp3_ = g_thread_create (_ipipe_aew_thread_func_gthread_func, self, TRUE, &_inner_error_);
	_tmp4_ = _tmp3_;
	if (_inner_error_ != NULL) {
...
static void _dbus_ipipe_init_aew (Ipipe* self, GVariant* parameters, GDBusMethodInvocation* invocation) {
	...
	result = ipipe_init_aew (self, wb, ae, g, meter, time, fps, segment_factor, width, height, &error);
	if (error) {
		g_dbus_method_invocation_return_gerror (invocation, error);
		return;
	}
